...
- 简历列表：创建、复制、删除多份简历版本。
- 求职进展工作台：记录公司、岗位、所在地、截止时间和联系人，并关联简历版本。
- 阶段时间线：申请阶段在 待投递 → 已投递 → 笔试 → 面试 → Offer → 结束 之间相邻推进或回退，每次变化自动写入带时间的时间线。
- 关联保护：简历删除或复制不影响申请历史；关联丢失时保留申请，并提供重新关联入口。
...
- 本地持久化：简历、岗位申请、个人资料、主题偏好和模板偏好存储在 localStorage。
- JSON 导入导出：支持完整工作区备份和恢复（含申请、时间线与简历关联），兼容缺少新字段的旧备份。
